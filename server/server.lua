...
AddEventHandler("onResourceStart", function(resource)
    if resource == GetCurrentResourceName() then
        versionCheck('kangoka/alyf_vips')
    end
end)

RegisterCommand('generar', function(source)
    TriggerClientEvent("openDialogCreate", source)
end, true)

RegisterCommand('compra', function(source)
    TriggerClientEvent("openDialogRedeem", source)
end, false)

ESX.RegisterServerCallback('alyf_vips:crearsorteo', function(source, cb, data, code)
    ...
    if data[1] ~= nil and data[1] ~= '' then
        ...
        MySQL.query('SELECT code FROM alyf_store_codes WHERE code = ?', { data[1] }, function(result)
            if #result > 0 then
                -- Update the data if the code is exists
                MySQL.update('UPDATE alyf_store_codes SET code = ?, maxuse = ?, reward = ?, sameuser = ?, quantity = ? WHERE code = ?'
                ...
                MySQL.insert('INSERT INTO alyf_store_codes (code, maxuse, reward, sameuser, quantity) VALUES (?, ?, ?, ?, ?)'
                    ,
...
ESX.RegisterServerCallback('alyf_vips:pedirsorteo', function(source, cb, data)
    if data[1] == nil then
        return cb('empty')
    end
    ...
    MySQL.query('SELECT code, maxuse, sameuser, reward, quantity FROM alyf_store_codes WHERE code = ?', { data[1] },
        function(result)
            if #result > 0 then
                MySQL.query('SELECT identifier, code FROM alyf_store_logs WHERE code = ?', { data[1] },
                    function(result2)
                        ...
                        if #result2 + 1 > result[1].maxuse then
                            ...
                            MySQL.insert('INSERT INTO alyf_store_logs (identifier, code) VALUES (?, ?)',
